...
if ($result -> num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        ...
        array_push($values, substr($row['Timestamp'], 0, -9));
        array_push($values, substr($row['Open_USD'], 0, -3));
        array_push($values, substr($row['High_USD'], 0, -3));
        array_push($values, substr($row['Low_USD'], 0, -3));
        array_push($values, substr($row['Close_USD'], 0, -3));
        array_push($values, substr($row['Volume'], 0, -3));
...
<head>
    <meta charset="utf-8">
    <link href="../graphing/build/nv.d3.css" rel="stylesheet" type="text/css">
    <script src="https://cdnjs.cloudflare.com/ajax/libs/d3/3.5.17/d3.min.js" charset="utf-8"></script>
    <script src="../graphing/build/nv.d3.js"></script>
...
//         {"date": 15953, "open": 165.85, "high": 166.4, "low": 165.73, "close": 165.96, "volume": 62930500, "adjusted": 165.96}
    ...
    nv.addGraph(function() {
        ...
        var chart = nv.models.candlestickBarChart()
            .x(function(d) { return format.parse(d['date']) })
...
//                  	return d3.time.format('%x')(new Date(new Date() - (20000 * 86400000) + (d * 86400000)));
                 	return d3.time.format('%x')(new Date(new Date(d)));
                });
